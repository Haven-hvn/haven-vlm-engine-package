...
class TestActionRange:
    ...
    def test_action_range_initialization(self):
        ...
        action_range = ActionRange(
            start_frame=10, end_frame=100, action_tag="test_action"
        )
...
class TestVideoFrameExtractor:
    ...
    def test_extract_frame_decord_success(self, mock_video_reader):
        ...
        with patch(
            "vlm_engine.binary_search_processor.crop_black_bars_lr",
...
class TestParallelBinarySearchEngine:
    ...
    async def test_process_video_binary_search_mock(self):
        ...
        assert self.engine.api_calls_made > 0
        assert (
            self.engine.api_calls_made < total_frames
        )  # Should be much less than linear
    ...
    async def test_process_video_binary_search_invalid_metadata(self):
        ...
        with (
            ...
            results = await self.engine.process_video_binary_search(
                video_path="test_video.mp4", vlm_analyze_function=mock_vlm_analyzer
            )
...
class TestVLMBatchCoordinator:
    ...
    def test_get_performance_stats_with_data(self):
        ...
        self.coordinator.total_frames_processed = 10
        self.coordinator.start_time = 100.0
...
async def test_progress_callback_called():
    ...
    engine = ParallelBinarySearchEngine(
        action_tags=["test_action"], progress_callback=Mock()
    )
    ...
    assert all(
        p1 < p2 for p1, p2 in zip(progresses, progresses[1:])
    )  # Strictly increasing
...
class TestIntegrationScenarios:
    ...
    async def test_binary_search_performance_vs_linear(self):
        ...
        print(
            f"Improvement: {((linear_api_calls - binary_search_calls) / linear_api_calls * 100):.1f}%"
        )

        # Binary search should be significantly more efficient
        assert binary_search_calls < linear_api_calls
        assert (
            binary_search_calls < total_frames * 0.1
        )  # Should be less than 10% of total frames
...
def run_performance_benchmark():
    ...
    for scenario in test_scenarios:
        ...
        improvement = (
            ((linear_calls - binary_calls) / linear_calls * 100)
            if linear_calls > 0
            else 0
        )
