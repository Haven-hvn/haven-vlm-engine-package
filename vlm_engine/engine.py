...
class VLMEngine:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.debug(
                "[DEBUG_VLM_ENGINE] Pipeline initialization completed successfully"
            )
        except Exception as e:
            logger.error(f"[DEBUG_VLM_ENGINE] Pipeline initialization failed: {e}")
            logger.debug(
                f"[DEBUG_VLM_ENGINE] Exception traceback: {traceback.format_exc()}"
            )
            raise
    ...
    async def process_video(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.debug(
                f"[DEBUG_VLM_ENGINE] Getting request future for pipeline: {pipeline_name}"
            )
            future = await self.pipeline_manager.get_request_future(
                data, pipeline_name, callback=progress_callback
            )
            ...
            logger.debug(
                f"[DEBUG_VLM_ENGINE] Video processing completed in {duration:.2f}s"
            )
            ...
            duration = time.time() - start_time
            logger.error(
                f"[DEBUG_VLM_ENGINE] Video processing failed after {duration:.2f}s: {e}"
            )
            logger.debug(
                f"[DEBUG_VLM_ENGINE] Exception traceback: {traceback.format_exc()}"
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        """Get performance statistics from the pipeline manager."""
        return self.pipeline_manager.get_performance_stats()
