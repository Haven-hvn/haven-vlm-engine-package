...
class Pipeline:
    def __init__(
        ...
        for model_config in config.models:
            ...
            if modelName == "dynamic_video_ai":
                dynamic_models: List[ModelWrapper] = (
                    dynamic_ai_manager.get_dynamic_video_ai_models(
                        model_inputs,
                        model_outputs
            ...
            self.models.append(
                ModelWrapper(
                    ...
                    model_inputs,
                    model_outputs,
                    model_name_for_logging=modelName,
                )
            )

        categories_set: Set[str] = set()
        for wrapper_model in self.models:
            if hasattr(wrapper_model.model, "model") and isinstance(
                wrapper_model.model.model, AIModel
            ):
                current_categories: Union[str, List[str], None] = (
                    wrapper_model.model.model.model_category
                )
                if isinstance(current_categories, str):
                    if current_categories in categories_set:
                        raise ValueError(
                            f"Error: AI models must not have overlapping categories! Category: {current_categories}"
                        )
                    ...
                    for cat in current_categories:
                        if cat in categories_set:
                            raise ValueError(
                                f"Error: AI models must not have overlapping categories! Category: {cat}"
                            )
                        categories_set.add(cat)

        is_vlm_pipeline: bool = any(
            isinstance(mw.model.model, VLMAIModel)
            for mw in self.models
            if hasattr(mw.model, "model")
        )
        if is_vlm_pipeline:
            for model_wrapper in self.models:
                if hasattr(model_wrapper.model, "model") and isinstance(
                    model_wrapper.model.model, VideoPreprocessorModel
                ):
        ...
        for current_model_wrapper in self.models:
            if key in current_model_wrapper.inputs:
                ...
                for inputName in current_model_wrapper.inputs:
                    if inputName != key:
                        is_present = (
                            itemFuture.data is not None and inputName in itemFuture.data
                        )
                        if not is_present:
                ...
                if allOtherInputsPresent:
                    await current_model_wrapper.model.add_to_queue(
                        QueueItem(
                            itemFuture,
                            current_model_wrapper.inputs,
                            current_model_wrapper.outputs,
                        )
                    )

    async def start_model_processing(self) -> None:
        for model_wrapper in self.models:
            if hasattr(model_wrapper.model, "start_workers") and callable(
                model_wrapper.model.start_workers
            ):
    ...
    def get_ai_models_info(
        ...
        for model_wrapper in self.models:
            if hasattr(model_wrapper.model, "model") and isinstance(
                model_wrapper.model.model, AIModel
            ):
...
class PipelineManager:
    def __init__(
        self,
        pipelines_config: Dict[str, PipelineConfig],
        model_manager: ModelManager,
        category_config: Dict,
        dynamic_ai_manager: DynamicAIManager,
    ):
        ...
        for pipeline_name, pipeline_config in self.pipelines_config.items():
            ...
            try:
                new_pipeline = Pipeline(
                    ...
                    self.dynamic_ai_manager,
                )
                self.pipelines[pipeline_name] = new_pipeline
                await new_pipeline.start_model_processing()
                self.logger.info(
                    f"Pipeline {pipeline_name} V{new_pipeline.version} loaded successfully!"
                )
    ...
    async def get_request_future(
        ...
        itemFuture: ItemFuture = await ItemFuture.create(
            None, futureData, pipeline.event_handler
        )
        ...
        for pipeline in self.pipelines.values():
            for model_wrapper in pipeline.models:
                if hasattr(model_wrapper.model, "model"):
                    ...
                    if (
                        hasattr(inner_model, "batch_coordinator")
                        and inner_model.batch_coordinator
                    ):
