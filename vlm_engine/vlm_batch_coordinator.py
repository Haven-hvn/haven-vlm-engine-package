...
class VLMBatchCoordinator:
    ...
    def __init__(
        self, vlm_client: Union[OpenAICompatibleVLMClient, MultiplexerVLMClient]
    ):
        ...
        self.logger.info(
            f"VLMBatchCoordinator initialized with {type(vlm_client).__name__}"
        )
    ...
    async def analyze_frame(self, frame: Image.Image) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.debug(
                f"VLM analysis completed in {response_time:.2f}s, total calls: {self.total_calls}"
            )
            return result
            ...
            if isinstance(result, Exception):
                ...
                if hasattr(self.vlm_client, "tag_list"):
                    processed_results.append(
                        {tag: 0.0 for tag in self.vlm_client.tag_list}
                    )
                else:
                    processed_results.append({})
            else:
...
def get_vlm_model(
    config: Dict[str, Any],
) -> Union[OpenAICompatibleVLMClient, MultiplexerVLMClient]:
    """
    ...
    def _initialize_vlm_client(self) -> None:
        ...
        for model_wrapper in self.pipeline_models:
            if hasattr(model_wrapper.model, "model") and hasattr(
                model_wrapper.model.model, "vlm_model"
            ):
...
class MockVLMCoordinator:
    ...
    def __init__(
        self, action_tags: List[str], mock_responses: Optional[Dict[str, float]] = None
    ):
        ...
        self.logger.info(
            f"MockVLMCoordinator initialized with {len(action_tags)} action tags"
        )
    ...
    async def analyze_frame(self, frame: Image.Image) -> Dict[str, float]:
        ...
        self.logger.debug(
            f"Mock VLM analysis call #{self.call_count}: {len([k for k, v in result.items() if v > 0.5])} detections"
        )
        return result
